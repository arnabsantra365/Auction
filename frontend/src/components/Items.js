...
export default function Items({ product ,onSelect,isSelected}) {
  return (
   
<div className="min-w-screen h-screen animated fadeIn faster  left-0 top-0 flex justify-center items-center inset-0  outline-none focus:outline-none bg-no-repeat bg-center bg-cover"  id="modal-id">
    ...
    <div className="container">
      <div className="max-w-md w-full bg-gray-900 shadow-lg rounded-xl p-6">
        <div className="flex flex-col ">
          <div className="">
            <div className=" h-62 w-full mb-3 flex">
              <div className="productname">
                {/* <h1>{product.name}</h1> */}
                <h1 className='text-white'>{product.name}</h1>
              </div>
              {/* <div className=" flex flex-col top-0 right-0 p-3 ">
            ...
            <div className="flex-auto justify-evenly">
              <div className="flex flex-wrap ">
                ...
                <div className="flex items-center w-full justify-between min-w-0 ">
                  <h2 className="text-lg mr-auto cursor-pointer text-gray-200 hover:text-purple-500 truncate ">{product.description}</h2>
                  <div className="flex items-center bg-green-400 text-white text-xs px-2 py-1 ml-3 rounded-lg">
                    INSTOCK</div>
                </div>
              </div>
              <div className="text-xl text-white font-semibold mt-1">${product.basePrice}</div>
              ...
              <div className="flex space-x-2 text-sm font-medium justify-start">
                <button className={`transition ease-in duration-300 inline-flex items-center text-sm font-medium mb-2 md:mb-0 ${isSelected? 'bg-blue-500':'bg-purple-500'} px-5 py-2 hover:shadow-lg tracking-wider text-white rounded-full ${isSelected?'hover:bg-blue-600':'hover:bg-purple-600'} `}
